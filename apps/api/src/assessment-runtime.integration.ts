...
async function main(){
 ...
 try{
  ...
  const training=randomUUID(),version=randomUUID(),moduleId=randomUUID();
  ...
  await db.pool.query('insert into assessment_question_snapshots(tenant_id,assessment_id,question_id,question_version_id,position,prompt,options_json,correct_option_index,points) values($1,$2,$3,$4,1,$5,$6::jsonb,1,1)',[tenant,assessment,question,qv,'Correct option?',JSON.stringify(['No','Yes'])]);
