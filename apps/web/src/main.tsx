...
function App() {
  ...
  function changeRole(nextRole: WebRole) { setRole(nextRole); setHref(defaultScreenFor(nextRole).href); setViewState('success'); if (session.status === 'authenticated') setSession(demoSession('authenticated', nextRole)); }
  function changeSession(nextStatus: SessionStatus) { setSession(demoSession(nextStatus, role)); }
