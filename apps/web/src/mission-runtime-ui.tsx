...
export function LearnerAssessmentRuntimeView(){
 ...
 async function answer(q:AssessmentQuestion,index:number){if(!attempt)return;const previous=attempt;const next={...attempt,answers:[...attempt.answers.filter(a=>a.questionVersionId!==q.questionVersionId),{questionVersionId:q.questionVersionId,selectedOptionIndex:index}]};setError(null);setAttempt(next);try{await http.put(`/api/v1/attempts/${encodeURIComponent(previous.id)}/answers/${encodeURIComponent(q.questionVersionId)}`,{selectedOptionIndex:index});}catch(e){setAttempt(current=>current?.id===previous.id?previous:current);setError(err(e));}}
